...
internal static partial class Program
{
    private static void ValidateTaskProgressCompatibility()
    {
        ...
        MethodInfo record = RequiredMethod(taskModule, "RecordTableDrivenProgress", BindingFlags.Static | BindingFlags.NonPublic,
            [typeof(Session), typeof(IEnumerable<(int ConditionType, int? Parameter, int Amount)>), typeof(bool), RequiredAscNetGameServerType("AscNet.GameServer.Handlers.TheatreModule+Mutation"), RequiredAscNetGameServerType("AscNet.GameServer.Handlers.Theatre5Module+Mutation")]);
        int packetId = 47_600;
        ValidateLoginDays();
        ...
        for (int rosterIndex = 0; rosterIndex < 2; rosterIndex++)
        {
            ...
            record.Invoke(null, [harness.Session, new (int, int?, int)[] { (11202, 4, 17) }, true, null, null]);
            ...
            record.Invoke(null, [harness.Session, new (int, int?, int)[] { (29018, null, 1), (29004, null, 1) }, true, null, null]);
